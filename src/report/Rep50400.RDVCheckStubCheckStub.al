report 50400 "RDV Check (Stub/Check/Stub)"
{
    DefaultLayout = RDLC;
    RDLCLayout = './layout-rdlc/Rep50100.RDVCheck(StubCheckStub).rdlc';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            //nj20201021 - Start
            column(PrintWithSignature; PrintWithSignature)
            {
            }
            column(FirstSignature_BankAcc2; BankAcc2."SCP First Signature")
            {
            }
            column(SecondSignature_BankAcc2; BankAcc2."SCP Second Signature")
            {
            }
            //nj20201021 - End
            dataitem(CheckPages; "Integer")
            {
                ...
                //nj20201029 - Start
                column(CheckNoTextMICR; '(' + CheckNoTextMICR + ')')
                {
                }
                column(CheckPrefix; CheckPrefix)
                {
                }
                //nj20201029 - End
                dataitem(PrintSettledLoop; "Integer")
                {
                    ...
                    //
                    //nj20201029 - Start
                    column(BankAcctNo; BankAccountNo + ')')
                    {
                    }
                    column(RoutingNumber; '0' + BankAcc2."Bank Code" + BankAcc2."Transit No.")
                    {
                    }
                    column(Transit_Institution; 'A' + BankTransitNo + 'A')
                    {
                    }
                    //nj20201029 - End
                    ...
                    begin
                        ...
                        BankTransitNo := BankAcc2."Transit No." + '-' + BankAcc2."Bank Branch No.";
                ...
                dataitem(PrintCheck; "Integer")
                {
                    ...
                    //nj20201029 - Start
                    column(CheckNoText_MICR; '(' + CheckNoText)
                    {
                    }
                    //nj20201029 - End
                    ...
                    //nj20201021 - Start
                    column(CheckDateChar1; CheckDateChar[1]) { }
                    column(CheckDateChar2; CheckDateChar[2]) { }
                    column(CheckDateChar3; CheckDateChar[3]) { }
                    column(CheckDateChar4; CheckDateChar[4]) { }
                    column(CheckDateChar5; CheckDateChar[5]) { }
                    column(CheckDateChar6; CheckDateChar[6]) { }
                    column(CheckDateChar7; CheckDateChar[7]) { }
                    column(CheckDateChar8; CheckDateChar[8]) { }
                    column(BankAcc2Addr1; BankAcc2Addr[1]) { }
                    column(BankAcc2Addr2; BankAcc2Addr[2]) { }
                    column(BankAcc2Addr3; BankAcc2Addr[3]) { }
                    column(BankAcc2Addr4; BankAcc2Addr[4]) { }
                    column(BankAcc2Addr5; BankAcc2Addr[5]) { }
                    column(BankAcc2Addr6; BankAcc2Addr[6]) { }
                    column(BankAcc2Addr7; BankAcc2Addr[7]) { }
                    column(BankAcc2Addr8; BankAcc2Addr[8]) { }
                ...
                begin
                    ...
                    //nj20201029 - Start
                    CheckNoTextMICR := CheckNoText;
                    IF STRLEN(CheckNoTextMICR) > 8 THEN
                        CheckNoTextMICR := COPYSTR(CheckNoTextMICR, STRLEN(CheckNoTextMICR), 8)
                    ELSE
                        WHILE STRLEN(CheckNoTextMICR) < 8 DO
                            CheckNoTextMICR := '0' + CheckNoTextMICR;
                    //nj20201029 - End                    
                end;
            ...
            begin
                ...
                if not TestPrint then begin
                    ...
                    //nj20201021 - Start
                    CheckDate := FORMAT("Posting Date", 0, '<Day,2><Month,2><Year4>');
                    CheckDateChar[1] := COPYSTR(CheckDate, 3, 1);  //M
                    CheckDateChar[2] := COPYSTR(CheckDate, 4, 1);  //M
                    CheckDateChar[3] := COPYSTR(CheckDate, 1, 1);  //D
                    CheckDateChar[4] := COPYSTR(CheckDate, 2, 1);  //D
                    CheckDateChar[5] := COPYSTR(CheckDate, 5, 1);  //Y
                    CheckDateChar[6] := COPYSTR(CheckDate, 6, 1);  //Y
                    CheckDateChar[7] := COPYSTR(CheckDate, 7, 1);  //Y
                    CheckDateChar[8] := COPYSTR(CheckDate, 8, 1);  //Y
                    //nj20201021 - End                      
                end else begin
            ...
            var
                ...
                lrecTempGenJnlLine: Record "Gen. Journal Line" temporary;  //nj20201021
            begin
                Copy(VoidGenJnlLine);
                CompanyInfo.Get();
                if not TestPrint then begin
                ...
                //nj20201021 - Start
                IF NOT TestPrint THEN BEGIN
                    //PrintWithSignature := TRUE;
                    lrecTempGenJnlLine.RESET;
                    lrecTempGenJnlLine.DELETEALL;
                    GenJnlLine2.RESET;
                    GenJnlLine2.SETRANGE("Account Type", GenJnlLine2."Account Type"::Vendor);
                    //GenJnlLine2.SETRANGE("Source No.", 
                    GenJnlLine2.SETRANGE("Bank Payment Type", "Bank Payment Type"::"Computer Check");
                    GenJnlLine2.SETRANGE("Check Printed", FALSE);
                    IF OneCheckPrVendor THEN BEGIN
                        IF GenJnlLine2.FINDSET THEN BEGIN
                            REPEAT
                                lrecTempGenJnlLine.RESET;
                                lrecTempGenJnlLine.SETRANGE("Journal Template Name", GenJnlLine2."Journal Template Name");
                                lrecTempGenJnlLine.SETRANGE("Journal Batch Name", GenJnlLine2."Journal Batch Name");
                                lrecTempGenJnlLine.SETRANGE("Source Type", GenJnlLine2."Source Type"::Vendor);
                                lrecTempGenJnlLine.SETRANGE("Source No.", GenJnlLine2."Source No.");
                                IF NOT lrecTempGenJnlLine.FINDFIRST THEN BEGIN
                                    lrecTempGenJnlLine := GenJnlLine2;
                                    lrecTempGenJnlLine.INSERT;
                                END ELSE BEGIN
                                    lrecTempGenJnlLine.Amount += GenJnlLine2.Amount;
                                    lrecTempGenJnlLine.MODIFY;
                                END;
                            UNTIL GenJnlLine2.NEXT = 0;
                            lrecTempGenJnlLine.RESET;
                            IF lrecTempGenJnlLine.FINDSET THEN
                                REPEAT
                                    GenJnlLine2.SETRANGE("Journal Template Name", lrecTempGenJnlLine."Journal Template Name");
                                    GenJnlLine2.SETRANGE("Journal Batch Name", lrecTempGenJnlLine."Journal Batch Name");
                                    GenJnlLine2.SETRANGE("Source Type", lrecTempGenJnlLine."Source Type"::Vendor);
                                    GenJnlLine2.SETRANGE("Source No.", lrecTempGenJnlLine."Source No.");
                                    IF BankAcc2."SCP Max Amount with Signature" >= lrecTempGenJnlLine.Amount THEN
                                        GenJnlLine2.MODIFYALL("SCP Print with Sign. Approved", TRUE)
                                    ELSE
                                        GenJnlLine2.MODIFYALL("SCP Print with Sign. Approved", FALSE);
                                UNTIL lrecTempGenJnlLine.NEXT = 0;
                        END;
                    END ELSE BEGIN
                        IF GenJnlLine2.FINDSET THEN
                            REPEAT
                                IF BankAcc2."SCP Max Amount with Signature" >= GenJnlLine2.Amount THEN
                                    GenJnlLine2."SCP Print with Sign. Approved" := TRUE
                                ELSE
                                    GenJnlLine2."SCP Print with Sign. Approved" := FALSE;
                                GenJnlLine2.MODIFY;
                            UNTIL GenJnlLine2.NEXT = 0;
                    END;
                END;
                FormatAddr.BankAcc(BankAcc2Addr, BankAcc2);
                BankAcc2Addr[1] := BankAcc2."AMC Bank Name";  //nj2020113
                IF PrintWithSignature THEN BEGIN
                    BankAcc2.CALCFIELDS("SCP First Signature", "SCP Second Signature");
                    SETRANGE("SCP Print with Sign. Approved", TRUE);
                END ELSE
                    SETRANGE("SCP Print with Sign. Approved");
                CheckPrefix := 'OSCI1-';
                CurrencyName := '';
                if (BankAcc2."Currency Code" = '') or (BankAcc2."Currency Code" = 'CAD') then
                    CurrencyName := 'Canadian'
                else
                    if BankAcc2."Currency Code" = 'USD' then
                        CurrencyName := 'US';
                //nj20201021 - End
            end;
            //GenJnlLine - End
    ...
    var
        ...
        //nj20201021 - Start
        CheckDateChar: array[8] of Text[1];
        CheckDate: Text[30];
        BankAcc2Addr: array[8] of Text[100];
        PrintWithSignature: Boolean;
        CheckPrefix: Text[10];
        ChequeNoCaptionLbl: Label 'Cheque Number';
        ChequeDateCaptionLbl: Label 'Cheque Date';
        CurrencyName: Text[20];
        CheckNoTextMICR: Text[30];
    ...
    //nj20201021 - Start
    procedure SetPrintWithSignature()
    begin
        PrintWithSignature := true;
    end;
    //nj20201021 - End
